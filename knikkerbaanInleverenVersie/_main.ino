...
Teller tellerA = Teller(TELLER_A_PIN); // voor teller A
Teller tellerB = Teller(TELLER_B_PIN); // voor teller B
Teller tellerC = Teller(TELLER_C_PIN); // voor teller C
...
//  pinMode(BUZZERPINB, OUTPUT);
  poortBoven.begin(BOVEN_POORT_PIN, 90, 10);
...
//laat de teller B detecteren:
    ...
    tellerC.update();
    if (vorigeAantalknikkersC < tellerC.getAantal()) {
      // digitalWrite(BUZZERPIN, HIGH);
      tone(BUZZERPIN, toonBuzzer, 250);
      Serial.println("zet de buzzer aan");
    ...
    vorigeAantalknikkersC = tellerC.getAantal();
    ...
    if (millis() > tijdVoorContactMetServer + LEEGLOOP_TIJD) {
      ...
      Serial.println(tellerC.getAantal());
